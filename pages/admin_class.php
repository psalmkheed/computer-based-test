...
<body>
    ...
    <th>S/N</th>
        ...
        <td>{$row['id']}</td>
            <td>{$row['class']}</td>
            <td style='text-transform: uppercase;'>{$row['session']}</td>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header bg-success-subtle">
                            <h5 class="modal-title text-success" id="addExamModalLabel">Create New Class
                            </h5>
                        ...
                        <div class="modal-body">
                            <form id="classForm" method="POST">
                                ...
                                <div class="col mb-3">
                                    <label for="session" class="form-label">Active Session</label>
                                    <select class="form-control bg-success text-white" name="session" required>
                                        <option value="2024/2025" selected> 2024/2025</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="teacher" class="form-label">Teacher</label>
                                    <select name="teacher" id="teacher" class="form-select mb-3" required>
                                        <?php
                                        include '../connection/db_connection.php';
                                        $result = $conn->query("SELECT Staff_name FROM staff_registration");
                                        while ($row = $result->fetch_assoc()) {
                                            echo "<option value='{$row['Staff_name']}'>" . htmlspecialchars($row['Staff_name']) . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
        ...
        <script>
            $(document).ready(function () {
                $("#classForm").on("submit", function (e) {
                    ...
                    $.ajax({
                        url: "/CBT/connection/add_class.php",
                        type: "POST",
                        data: formData,
